...
public:
    ...
    /**
     * @brief SPI configuration structure for ESP32-S3.
     *
     * The example uses `SPI3_HOST` so that SPI2 (with its native IOMUX
     * pins on GPIO9-14) stays free for any other peripheral that
     * benefits from IOMUX. SPI3 has no IOMUX path on ESP32-S3 — every
     * pin (including CS) is routed through the GPIO matrix, which is
     * exactly what we want when the bus pins live in the GPIO 32+
     * range.
     */
    struct SPIConfig {
        spi_host_device_t host = SPI3_HOST;     ///< SPI host (SPI3 via GPIO matrix)
